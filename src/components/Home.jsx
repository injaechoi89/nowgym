import { TRAINERS, fmt, fmtM, ML } from '../data.js'
...
export default function Home({role, myTrainer}) {
  ...
  const prev = MONTH_SALES[prevKey]
    ...
    <div>
      ...
      {isOwner ? (
        ...
        <div className="metric-grid" style={{gridTemplateColumns:'repeat(3,minmax(0,1fr))'}}>
          <div className="metric"><div className="metric-label">{ML[curMonth-1]} 내 PT 매출</div><div className="metric-val g">{fmtM(myAmt)}</div><div className="metric-sub">인센 +{fmt(Math.round(myAmt*0.12))}</div></div>
